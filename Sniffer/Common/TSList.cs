...
namespace Sniffer.Common
{
	...
	public delegate void CancelNewEventHandler(int index);
	...
	[Serializable]
	public class TSList<T> : IList<T>, ICollection<T>, IEnumerable<T>, IBindingList, IRaiseItemChangedEvents, ICloneable, ISortable, ICancelAddNew
	{
		[NonSerialized]
		...
		private void onListChanged(ListChangedType listChangedType, T[] removedItems, T[] addedItems, int[] itemIndexes)
		{
			...
			if (IsSorted && AutoSort && addedItems.Length > 0)
			{
		...
		{
			if (IsSorted && AutoSort && sortProperty.Name == e.PropertyName)
			{
				ApplySort(sortProperty, sortDirection);
			}
		...
		public event CancelNewEventHandler OnCancelNew;
