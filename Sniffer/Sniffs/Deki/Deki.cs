...
namespace Sniffer.Sniffs.Deki
{
    public class Deki : Sniff
    {
        ...
        private static DekiUserStateDictionary userStates = new DekiUserStateDictionary();
        ...
        public Deki(HoveWebBrowser webBrowser)
            : base(webBrowser)
        {
        ...
        static Deki()
        {
            ...
            try
            {
                userStates = (DekiUserStateDictionary)serializer.Deserialize("Sniffs/Deki/DekiUsersStates.xml");
            }
            catch { }
        ...
        {
            SharpSerializer serializer = new SharpSerializer();
            serializer.Serialize(userStates, "Sniffs/Deki/DekiUsersStates.xml");
        }
        ...
        private void cleanUsersStates()
        {
            ...
            foreach (KeyValuePair<string, DekiUserState> userState in userStates)
            {
        ...
        private bool process(object state)
        {
            try
            {
                ...
                foreach (DekiUserSettings userSettings in Settings.UserSettings)
                {
                    if (!userSettings.IsActive) continue;

                    if (!userStates.ContainsKey(userSettings.Key))
                        userStates[userSettings.Key] = new DekiUserState();

                    DekiUserState userState = userStates[userSettings.Key];
        ...
        private List<Post> getPosts()
        {
            ...
            for (int i = 0; i < titles.Count; i++)
            {
                ...
                if (hrefIndex != -1)
                {
                    titleHtml = titleHtml.Insert(hrefIndex, "http://deki11.bloger.index.hr");
                }
                ...
                foreach (GeckoHtmlElement text in blogText.GetElementsByTagName("div"))
                {
        ...
        private void generateAndSendMail(DekiUserSettings userSettings)
        {
            DekiUserState userState = userStates[userSettings.Key];

            if (userState.PostsToSend.Count == 0) return;

            string subject = string.Concat("Deki prognoze (", userState.PostsToSend.Count, ")");
        ...
        private string generateHTMLFormat(DekiUserState userState)
        {
            StringBuilder body = new StringBuilder();

            body.AppendLine("<html><head></head><body>");
            body.AppendLine(@"<style type=""text/css"">");
            body.AppendLine(File.ReadAllText(Path.Combine(Application.StartupPath, @"Sniffs\Deki\Post.css")));
        ...
        {
            base.Start("http://deki11.bloger.index.hr/default.aspx");
        }
    }
